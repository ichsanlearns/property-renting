...
function HomePage() {
  ...
  return (
    <main>
      ...
      <section className="relative py-16 px-6 md:px-12 max-w-360 mx-auto z-20">
        <button
          onClick={scrollLeft}
        ...
        <div className="flex justify-between items-end mb-10">
          <div>
            <h2 className="text-3xl font-extrabold text-on-background tracking-tight">Stay somewhere unique</h2>
            <p className="text-on-surface-variant mt-1">Hand-picked homes for every traveler.</p>
          </div>
          <button onClick={() => navigate("/search")} className="text-[#ff5c61] font-bold flex items-center hover:underline cursor-pointer">
            View all
        ...
        <div ref={scrollRef} className="flex gap-8 overflow-x-auto">
          {properties?.map((p) => (
            <div key={p.id} className="w-100 h-full shrink-0">
      ...
      <section className="py-20 px-6 text-center max-w-4xl mx-auto">
        <h2 className="text-4xl font-extrabold text-on-background mb-6 tracking-tight">Ready to start your journey?</h2>
        <p className="text-lg text-on-surface-variant mb-10 font-medium">Join over 10 million travelers discovering the world's most unique stays with SewaHunian.</p>
