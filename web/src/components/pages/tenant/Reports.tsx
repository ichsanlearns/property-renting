function Reports() {
  const summaryCards = [
    { label: "Total Revenue", value: "$128,430.00", trend: "+12.5%", icon: "payments", color: "primary" },
    { label: "Total Bookings", value: "842", trend: "+5.2%", icon: "confirmation_number", color: "blue-500" },
    { label: "Active Properties", value: "12", trend: "0%", icon: "domain", color: "purple-500" },
    { label: "Avg. Occupancy Rate", value: "84%", trend: "+3.1%", icon: "percent", color: "amber-500" },
  ];

  const propertySales = [
    { name: "Skyline Penthouse", bookings: 156, revenue: "$34,200.00", avg: "$219.23", status: "High Yield", imgId: 8 },
    { name: "Ocean Breeze Villa", bookings: 204, revenue: "$48,960.00", avg: "$240.00", status: "Popular", imgId: 9 },
    { name: "Mountain Retreat", bookings: 92, revenue: "$18,400.00", avg: "$200.00", status: "Stable", imgId: 10 },
    { name: "Urban Loft Studio", bookings: 188, revenue: "$22,560.00", avg: "$120.00", status: "High Volume", imgId: 11 },
  ];
            ...
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-2 w-full group">
                <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-t-lg relative h-48">
                  <div className={`absolute bottom-0 w-full bg-primary/40 rounded-t-lg group-hover:bg-primary transition-all duration-300 ${bar.h}`}></div>
                </div>
                <span className="text-[10px] text-slate-400 rotate-45 origin-left truncate w-12">{bar.label}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              {propertySales.map((item, idx) => (
                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-lg bg-slate-200 bg-cover bg-center" style={{ backgroundImage: `url('http://googleusercontent.com/profile/picture/${item.imgId}')` }}></div>
